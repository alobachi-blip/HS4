...
The plugin requests `HScene` at the first safe frame, skips the normal title/selection/map flow, disables the transition fade/loading image, and randomly fills two female plus two male slots from:
...
Persistent mode and card/map settings are available in `BepInEx\config\com.hs2.directhlauncher.cfg` after the plugin has loaded once.
