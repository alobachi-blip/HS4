...
- 開協助時若已在高潮後：改走此路徑，不再立刻「進場抽姿」。
- HUD：僅在那 5s 顯示「高潮後欣賞」；開幹後當完整作愛流程，**不一直掛脫力**。

### HUD／相機／進場姿

- **欣賞鎖紅字**：`longAppreciation` 時協助列改紅色粗體「欣賞中：按 L／滾輪／N 可手動跳出」。
- **選單停轉**：游標在遊戲 UI、確認框、或 Ctrl+Shift+P 設定窗時暫停環視轉動（HUD 顯示「相機暫停：操作選單中」）。
- **進場抽姿**：開協助或進 H 時排程選池抽一次起始姿，避免總是預設接吻；失敗逾時則沿用當前姿。

## 2026-07-12

### 修正：Feel 0.001 被抬成 0.01；鏡頭跟動作晃動

- **Feel**：舊版會把小於 0.01 的值抬成 0.01，關窗後看起來像沒存。已允許 0～5（含 0.001）、用不變地區解析小數點，關閉視窗時強制寫入設定檔。
- **晃動**：改為換焦／換軸時鎖骨焦點相對身體根節點；圈內不跟頭／胸活骨頭（拿掉平滑）。全身位移仍跟，手部／姿勢動畫不晃鏡頭。
