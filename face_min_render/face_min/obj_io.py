...
def load_obj(path: str | Path) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Return verts (N,3), faces (F,3), uvs (N,2), uv_faces placeholder."""
    text = Path(path).read_text(encoding="utf-8", errors="ignore")
    verts, faces, uvs = load_obj_from_text(text)
    return verts, faces, uvs, np.zeros((0, 3), dtype=np.int32)


def load_obj_from_text(text: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    faces_a = np.asarray(faces_v, dtype=np.int32) if faces_v else np.zeros((0, 3), dtype=np.int32)
    ...
    if uvs_raw:
        ...
        for fv, ft in zip(faces_v, faces_t):
            for vi, ti in zip(fv, ft):
                if 0 <= ti < len(uvs_a) and 0 <= vi < len(verts):
                    acc[vi] += uvs_a[ti]
                    cnt[vi] += 1
        mask = cnt[:, 0] > 0
        uvs[mask] = acc[mask] / cnt[mask]
    return verts_a, faces_a, uvs
